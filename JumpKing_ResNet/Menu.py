...
class Menus:
	...
	def _load_audio(self, file):
		...
		for audio_name in os.listdir(file):
			...
			audios[re.match(r"[^.]+", audio_name).group()] = pygame.mixer.Sound("{}/{}".format(file, audio_name))
	...
	def _load_images(self, file):
		...
		for image_name in os.listdir(file):
			...
			if image_name == "frame.png":
				...
				image = SpriteSheet("{}/{}".format(file, image_name))
				...
				images[re.match(r"[^.]+", image_name).group()] = pygame.image.load("{}/{}".format(file, image_name)).convert_alpha()
...
class Stat_Menu(Menu):
	...
	def update(self):
		...
		self.buttons[0].text = self.buttons[0].font.render("ATTEMPT : {} SESSION : {}".format(attempt, session), True, (150, 150, 150))

		self.buttons[1].text = self.buttons[1].font.render("TIME : {}H {}M {}s".format(hours, minutes, seconds), True, (150, 150, 150))
